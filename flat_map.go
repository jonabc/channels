...
// the output channel.
func FlatMap[TIn any, TOut any, TOutSlice []TOut](ctx context.Context, inc <-chan TIn, mapFn func(context.Context, TIn) (TOutSlice, bool)) <-chan TOut {
	outc := make(chan TOut, cap(inc))

	go func() {
		defer close(outc)
		for in := range inc {
			outSlice, ok := mapFn(ctx, in)
			if !ok {
				continue
			}
...
func FlatMapValues[TIn any, TOut any, TOutSlice []TOut](ctx context.Context, inc <-chan TIn, mapFn func(context.Context, TIn) (TOutSlice, bool)) []TOut {
	outc := FlatMap(ctx, inc, mapFn)
